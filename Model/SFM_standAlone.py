...
def visualize(prev_container, curr_container, focal, pp, prev_frame_id):
    ...
    curr_sec.set_title('curr(' + str(prev_frame_id+1) + ')')
...
def init_SFM(prev_img_path, curr_img_path, pkl_path, prev_tfl, curr_tfl,prev_frame_id):
    # read data and run
    # curr_frame_id = 29
    # prev_frame_id = 28
    ...
    EM = np.eye(4)
    for i in range(prev_frame_id, prev_frame_id+1):
    ...
    visualize(prev_container, curr_container, focal, pp, prev_frame_id)
